...
Вот, например, функция, в которую мы ожидаем на вход целочисленный тип. Тогда можно указать его как возвращаемое
значение и не передавать тип (там по умолчанию как раз `void`, что логично в нашем случае). Если в функцию передали
