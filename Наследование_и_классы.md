...
```
Каждый элемент вектора - указатель на Human и Student, тогда объекты могут иметь разные типы, занимать разное место в памяти, а вектору норм, потому что он знает размер указателя. При удалении вектор вызовет деструктор у умных указателей, а они вызовут удаление у всех объектов.

Без указателя - slicing
...
int main() {
    ...
    {
        Student *s = new Student("Egor", "Suvorov", "MBD191");
        delete s;
    } // Все работает без утечек
    {
        Human *h = new Student("Egor", "Suvorov", "MBD191");
        delete h;  // UB: ~Human()
        // delete static_cast<Student*>(h);  // Would be OK.
...
Наследники никак не могут переопределять или дополнять поведение родителей, только создавать новые методы. То же самое и с деструкторами. Если мы пытаемся удалить Student, думая, что это Human, то вызовется деструктор именно Human, а не Student - UB.

```cpp
Human *h = new Student("A", "B", "C");
delete h; // UB: ~Human()
```
**using**
...
Foo -- структура с приватным конструктором, хотим написать фабрику. Вызвать внутри просто ```make\_unique``` не получится, тк он в свою очередь вызывает ```new```, ```new``` вызывает конструктор, который у нас приватный (```make\_unique``` не является частью Foo и не имеет к доступа к private).
В качестве решения можно было бы попробовать объявить стандартную функцию ```std::make\_unique``` другом, но мы не знаем ее точной реализации и не можем быть уверены, что ```make\_unique``` вызывает new напрямую -- вдруг придется давать доступ к private методам еще кому-то... 
**Правильный вариант** руками создать чистый указатель и завернуть его в ```unique\_ptr```.
```cpp
#include <memory>
...
**про работу cast, override, hiding, crocc-cast**
